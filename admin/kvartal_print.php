...
<body>
    ...
    if (isset($_POST['kvartal_btn'])) {
        ...
        $query = "SELECT so.idorderclient, w.worker, cl.client, CONCAT(m.name, ' ', a.model) as car, a.number, a.vin, oc.termorder, GROUP_CONCAT(DISTINCT s.service SEPARATOR '\n') as services, SUM(so.price_at_order) as order_amount FROM serviceorder so INNER JOIN service s ON so.idservice = s.idservice INNER JOIN orderclient oc ON so.idorderclient = oc.idorderclient INNER JOIN worker w ON oc.idworker = w.idworker INNER JOIN auto a ON oc.idauto = a.idauto INNER JOIN marka m ON a.idmarka = m.idmarka INNER JOIN client cl ON oc.idclient = cl.idclient INNER JOIN status st ON oc.idstatus = st.idstatus WHERE st.idstatus = 3 AND oc.termorder BETWEEN $kvartalSelect GROUP BY so.idorderclient, oc.termorder, w.worker, a.number, a.model, a.vin, cl.client, m.name";
                    ...
                    <table class="table table-bordered table-hover text-dark">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-center align-middle">Детали</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            while ($row = mysqli_fetch_assoc($query_run)) {
                                // Сумма услуг
                                $service_sum = $row['order_amount'];
                                // Сумма деталей
                                $idorderclient = $row['idorderclient'];
                                $details_query = "SELECT d.detail, do.count, do.price_at_order FROM detailorder do INNER JOIN detail d ON do.iddetail = d.iddetail WHERE do.idorderclient = '$idorderclient'";
                                $details_result = mysqli_query($connection, $details_query);
                                $details_arr = [];
                                $details_sum = 0;
                                while ($detail_row = mysqli_fetch_assoc($details_result)) {
                                    $details_arr[] = $detail_row['detail'] . ' (x' . $detail_row['count'] . ')';
                                    $details_sum += $detail_row['price_at_order'] * $detail_row['count'];
                                }
                                $details_str = $details_arr ? implode(', ', $details_arr) : 'Нет деталей';
                                // Итоговая сумма заказа
                                $order_total = $service_sum + $details_sum;
