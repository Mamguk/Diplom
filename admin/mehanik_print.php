...
<body>
    ...
    if (isset($_POST['mehanik_btn'])) {
        ...
        $query = "SELECT orderclient.idorderclient, orderclient.idworker, worker.worker, worker.phone, dateorder, orderclient.idstatus, status.status, termorder, orderclient.idauto, auto.number, auto.vin, CONCAT(marka.name, ' ', auto.model) as car, auto.year, auto.mileage, orderclient.idclient, client.client FROM orderclient INNER JOIN status ON orderclient.idstatus = status.idstatus INNER JOIN worker ON orderclient.idworker = worker.idworker INNER JOIN auto ON orderclient.idauto = auto.idauto INNER JOIN marka ON auto.idmarka = marka.idmarka INNER JOIN client ON orderclient.idclient = client.idclient where orderclient.idworker = $idworker ORDER BY orderclient.idorderclient ASC";
